      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
          ...
          <section className="grid gap-5">
            ...
            <div className="surface rounded-lg p-5"><AddressForm /><div className="mt-4 grid gap-2">{addresses.data?.map((address: { id: number; street: string; number: string; city: string; state: string }) => <p key={address.id} className="rounded-lg border border-ink/10 p-3">{address.street}, {address.number} - {address.city}/{address.state}</p>)}</div></div>
            <div className="surface rounded-lg p-5"><h2 className="text-xl font-black">Meus pedidos</h2><div className="mt-3 grid gap-3">{orders.data?.map((order) => <div key={order.id} className="flex items-center justify-between rounded-lg border border-ink/10 p-3"><strong>#{order.id} - {formatMoney(order.total)}</strong><OrderStatusBadge status={order.status} /></div>)}</div></div>
          </section>
        </div>
      </main>
      <Footer />

import { computeStaticTotal, STATIC_SHIPPING } from './commerce';
...
function computeGuestCart(entries: GuestEntry[]): GuestCart {
  ...
  return {
    ...
    shipping: toMoneyString(STATIC_SHIPPING),
    total: toMoneyString(computeStaticTotal(subtotalValue, discountValue)),
    is_guest: true,
  };
}

function withStaticShipping<T extends { subtotal: string; discount: string; shipping: string; total: string }>(cart: T): T {
  return {
    ...cart,
    shipping: toMoneyString(STATIC_SHIPPING),
    total: toMoneyString(computeStaticTotal(cart.subtotal, cart.discount)),
  };
}
...
export async function getCart(): Promise<CartLike> {
  if (isAuthenticated()) {
    ...
    return withStaticShipping(apiCart);
  }
...
export async function addToCart(product: Product, quantity = 1): Promise<CartLike> {
  if (isAuthenticated()) {
    ...
    return withStaticShipping(cart);
  }

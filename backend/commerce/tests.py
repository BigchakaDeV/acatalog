...
from .models import Address, Brand, Cart, Category, Coupon, Inventory, Order, Product, ProductImage, Review, Wishlist
...
class CommerceApiTests(TestCase):
    ...
    def test_coupon_recalculates_cart_total(self):
        ...
        self.assertEqual(response.data['total'], '99.75')
    ...
    def test_checkout_creates_snapshot_and_decrements_stock(self):
        ...
        self.assertEqual(order.shipping, Decimal('24.70'))
        self.assertEqual(order.total, Decimal('168.70'))
    ...
    def test_customer_can_review_product_once_and_rating_updates(self):
        self.auth()
        response = self.client.post('/api/reviews/', {'product': self.product.id, 'rating': 5, 'comment': 'Excelente'}, format='json')
        self.assertEqual(response.status_code, 201)
        self.product.refresh_from_db()
        self.assertEqual(self.product.average_rating, Decimal('5.00'))

        response = self.client.post('/api/reviews/', {'product': self.product.id, 'rating': 4, 'comment': 'Duplicada'}, format='json')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(Review.objects.filter(user=self.user, product=self.product).count(), 1)

    def test_customer_cannot_duplicate_wishlist_item(self):
        self.auth()
    ...
    def test_admin_can_import_product_images_by_url(self, mock_urlopen, _host_mock):
        ...
        mock_urlopen.side_effect = lambda *args, **kwargs: MockResponse(png_bytes)
